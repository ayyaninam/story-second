...
import {languages, videoLengths, videoRatios} from "@/features/generate/constants";
...
export const LanguageSelect = ({
  ...
  <Select defaultValue={selectedLanguage} onValueChange={(e) => setSelectedLanguage(e)}>
...
export const VideoLengthSelect = ({
  ...
  <Select defaultValue={selectedVideoLength} onValueChange={(e) => setSelectedVideoLength(e)}>
    <SelectTrigger className="w-full h-fit focus:ring-0 focus:ring-offset-0 px-2">
...
export const VideoRatioSelect = ({ selectedVideoRatio, setSelectedVideoRatio }:{
  selectedVideoRatio: string;
  setSelectedVideoRatio: (value: string) => void;
}) => (
  <Select defaultValue={selectedVideoRatio} onValueChange={(e) => setSelectedVideoRatio(e)}>
    <SelectTrigger className="w-full h-fit focus:ring-0 focus:ring-offset-0 px-2">
      <SelectValue placeholder="Video Ratio" />
    </SelectTrigger>
    <SelectContent>
      {videoRatios.slice(0, 2).map((ratio, index) => (
        <SelectItem key={index} value={ratio.value.replace(" ", "_").toLowerCase()}>
