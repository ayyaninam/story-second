  ...
  try {
    if (segment.type === "page") {
      if (segment.visual.format === "image") {
        await prefetch(segment.visual.imageURL, { method: "base64" }).waitUntilDone();
      }
      if (segment.visual.format === "gif") {
        await preloadGif(segment.visual.gifURL).waitUntilDone();
      }
      if (segment.audioURL) {
        await prefetch(segment.audioURL, { contentType: "audio/mpeg" }).waitUntilDone();
      }
    }

    if (segment.type === "intermediate") {
      if (segment.visual.format === "image") {
        await prefetch(segment.visual.imageURL, { contentType: "image/gif", method: "base64" }).waitUntilDone();
      }
      if (segment.visual.format === "gif") {
        await preloadGif(segment.visual.gifURL).waitUntilDone();
      }
...
export const prefetchAssets = async (segments: RemotionSegment[]) => {
  const prefetchAssetsPromises = segments.map((segment) => limit(() => prefetchSegmentAssets(segment)))
